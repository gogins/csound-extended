...
;------------------------------------------------------------
            ...
            instr       2
instances_  TestPlugin "$THREAD_NAME"
prints "There are %d instances of TestPlugin.\n", instances_
ishift      =           .00666667               ;shift it 8/1200.
ipch        =           cpspch(p5)              ;convert parameter 5 to cps.
ioct        =           octpch(p5)              ;convert parameter 5 to oct.
kvib        poscil      1/120, ipch/50, 1       ;vibrato
...
int render(const char *name) 
{
    ...
    output += name;
    output += ".wav";
    csound.SetOption(output.c_str());
    std::string macro = "--omacro:THREAD_NAME=";
    macro += name;
    csound.SetOption(macro.c_str());
    std::fprintf(stderr, "%-12.12s: csound.Compile...\n", name);
    csound.CompileCsdText(csd_text);
    std::fprintf(stderr, "%-12.12s: csound.Start...\n", name);
    csound.Start();
    std::fprintf(stderr, "%-12.12s: csound.Perform...\n", name);
    csound.Perform();
    std::fprintf(stderr, "%-12.12s: csound.Cleanup...\n", name);
    csound.Cleanup();
    std::fprintf(stderr, "%-12.12s: Finished.\n", name);
    return 0;
}
...
{
    std::thread one(render, "one");
    std::thread two(render, "two");
    one.join();
    two.join();
}
