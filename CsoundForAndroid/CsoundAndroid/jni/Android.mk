...
LOCAL_C_INCLUDES := $(LIBSNDFILE_SRC_DIR) $(HOME)/include $(CSOUND_SRC_ROOT)/H $(CSOUND_SRC_ROOT)/include $(CSOUND_SRC_ROOT) $(LIBSNDFILE_SRC_DIR) $(CSOUND_SRC_ROOT)/Engine $(CSOUND_SRC_ROOT)/interfaces $(NDK_MODULE_PATH)/oboe/include $(NDK_MODULE_PATH)/oboe/src $(NDK_MODULE_PATH)/oboe/src/common

ifeq ($(NDK_TOOLCHAIN_VERSION),clang)
LOCAL_CFLAGS := -std=c99 -Ofast -DENABLE_OPCODEDIR_WARNINGS -D__BUILDING_LIBCSOUND -DENABLE_NEW_PARSER -DLINUX -DHAVE_DIRENT_H -DHAVE_FCNTL_H -DHAVE_UNISTD_H -DHAVE_STDINT_H -DHAVE_SYS_TIME_H -DHAVE_SYS_TYPES_H -DHAVE_TERMIOS_H -DHAVE_STRTOK_R -DHAVE_BUILTIN_ATOMIC -DHAVE_PTHREAD -DHAVE_BUILTIN_ATOMIC -DHAVE_ATOMIC_BUILTIN -mllvm -unroll-allow-partial -mllvm -unroll-runtime -funsafe-math-optimizations -ffast-math  
else
