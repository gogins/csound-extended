...
void Counterpoint::ARRBLT(int *dest, int *source, int num) {
    int i;    /* SAIL p51 */
...
int Counterpoint::InMode(int Pitch, int Mode)
{
    ...
    case Dorian:
        ...
        break;
    case Phrygian:
        return(_Phrygian[pit]);
        break;
    case Lydian:
        return(_Lydian[pit]);
        break;
    case Mixolydian:
        return(_Mixolydian[pit]);
        break;
    case Locrian:
        return(_Locrian[pit]);
        break;
...
int Counterpoint::Us(int n, int v) {
    return(Ctrpt(n,v));
}
int Counterpoint::LastNote(int n, int v) {
    return(n == TotalNotes[v]);
}
int Counterpoint::FirstNote(int n, int v) {
    return(n == 1);
}
int Counterpoint::NextToLastNote(int n, int v) {
    return(n == (TotalNotes[v]-1));
}
void Counterpoint::SetUs(int n, int p, int v) {
    Ctrpt(n,v)=p;
}
...
int Counterpoint::Cantus(int n, int v) {
...
int Counterpoint::Other(int Cn, int v, int v1) {
...
int Counterpoint::DownBeat(int n, int v) {
    return(Beat8(Onset(n,v)) == 0);
}
int Counterpoint::UpBeat(int n, int v) {
    return(!(DownBeat(n,v)));
}
...
int Counterpoint::TooMuchOfInterval(int Cn, int Cp, int v)
{
    ...
    MinL=0;
    for (i=1; i<17; i++) {
        if ((i != k) && (Ints[i]>Ints[MinL])) MinL=i;
    }
...
int Counterpoint::Look(int CurPen, int CurVoice, int NumParts, int Species, int Lim, int *Pens, int *Is, int *CurNotes)
{
    ...
    for (Is[CurVoice]=1; Is[CurVoice]<=16; Is[CurVoice]++)
    {
        ...
        if (CurVoice == NumParts) tmp1=Species;
        else tmp1=1;
...
void Counterpoint::AnySpecies(int OurMode, int *StartPitches, int CurV, int CantusFirmusLength, int Species)
{
    ...
    if (CurV == 1) MaxPenalty=(2*RealBad);
    else MaxPenalty=infinity;
