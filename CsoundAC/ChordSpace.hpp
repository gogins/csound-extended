 ...
 */
SILENCE_PUBLIC const std::vector<std::vector<Chord> > &cyclical_region_sectors(bool transpositional_equivalence=true);
...
template<> inline SILENCE_PUBLIC Chord normalize<EQUIVALENCE_RELATION_RPTg>(const Chord &chord, double range, double g) {
    ...
    return normalize<EQUIVALENCE_RELATION_Tg>(voicing, range, g);
}
...
SILENCE_PUBLIC std::vector<int> cyclical_region_sector(const Chord &chord, bool transpositional_equivalence) {
    std::vector<Chord> sectors = cyclical_region_sectors(transpositional_equivalence)[chord.voices()];
...
SILENCE_PUBLIC const std::vector<std::vector<Chord> > &cyclical_region_sectors(bool transpositional_equivalence) {
    static bool initialized = false;
    static std::vector<std::vector<Chord>> cyclical_region_sectors_;
    static std::vector<std::vector<Chord>> cyclical_region_sectors_transpositional_equivalence;
    if (initialized == false) {
        for (auto dimensions = 3; dimensions <= 12; ++dimensions) {
            auto cyclical_region_vertices_ = cyclical_region_vertices(dimensions, false);
            auto cyclical_region_vertices_transpositional_equivalence = cyclical_region_vertices(dimensions, true);
            for (auto dimension = 0; dimension < dimensions; ++dimension) {
                auto center_ = cyclical_region_vertices_.front();
                auto center_transpositional_equivalence = center_.eT();
                *(cyclical_region_vertices_.end() - 1) = center_;
                *(cyclical_region_vertices_transpositional_equivalence.end() - 1) = center_transpositional_equivalence;
                cyclical_region_sectors_.push_back(cyclical_region_vertices_);
                cyclical_region_sectors_transpositional_equivalence.push_back(cyclical_region_vertices_transpositional_equivalence);
            }
        }
    }
    if (transpositional_equivalence == true) {
...
inline SILENCE_PUBLIC HyperplaneEquation hyperplane_equation_from_dimensionality(int dimensions, bool transpositional_equivalence, int sector_) {
    ...
    for (int sector = 0; sector < sector_; ++sector) {
        Chord vertex = cyclical_region_vertices_.front();
...
inline SILENCE_PUBLIC Chord reflect_in_inversion_flat(const Chord &chord) {
    ...
    HyperplaneEquation hyperplane = get_hyperplane_equation(dimensions);
