 ...
 * PLEASE NOTE: This only works if the chord already belongs to the scale! 
 * However, the original chord may be in any order or voicing. By default,
 * the chords are generated by thirds. If the original chord does not 
 * belong to the scale, the original chord is returned sorted.
 */
inline SILENCE_PUBLIC Chord transpose_degrees(const Chord &scale, const Chord &original_chord, int transpose_degrees, int interval = 3) {
    int voices = original_chord.voices();
    auto eop = original_chord.eOP();
    for (int scale_degree = 1; scale_degree < scale.voices(); ++scale_degree) {
        Chord test_eop = chord(scale, scale_degree, voices, interval).eOP();
        // Found the scale degree of the original chord, now get the transposed chord.
        if (eop == test_eop) {
            int target_degree = transpose_degrees + scale_degree;
            // Transposition has sign. If negative, wrap.
            while (target_degree < 1) {
                target_degree = target_degree + scale.voices();
            }
            Chord result = chord(scale, target_degree, voices);
            return result.eP();
        }
    }
    return original_chord.eP();
}

/**
 * Scale as a class; must be created with the name of the scale. Inherits 
 * from Chord. Note that Chord functions such as T and I still return Chords, 
 * not Scales.
 */
class SILENCE_PUBLIC Scale : public Chord {
    public:
        ...
        virtual Chord chord(int scale_degree, int voices, int interval = 3) {
            return csound::chord(*this, scale_degree, voices, interval);
        }
        virtual Chord transpose(const Chord &chord, int scale_degrees, int interval = 3) {
            return transpose_degrees(*this, chord, scale_degrees, interval);
        }
        virtual std::string name() const {
            return name_;
        }
