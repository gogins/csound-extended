...
function Koch(generator, t, d, c, k, v, level, score_)
    local t1 = t
    local d1 = d
    local c1 = c
    local k1 = k
    ...
    if level > 0 then
        for i = 1, #generator, 4 do
            ...
            d1 = d  * generator[i + 2]
            d2 = d1 * generator[i + 3]
            score_:append(Event:new{t1, d2, 144, c1, math.floor(k1), v1, 0, 0, 0, 0, 1})
            Koch(generator, t1, d1, c1, 12+k1, v1, level - 1, score_)
            t1 = t1 + d1
        end
    end
end
...
Koch(g, 1.00, duration, 1, 36, 74, 3, score)
Koch(g, 2.00, duration, 0, 48, 74, 3, score)
