...
(function () {
    ...
    */
    ...
    ParametricLindenmayer.Turtle.prototype.clone = function () {
        ...
        clone_.note = this.note.clone(true);
    ...
    ParametricLindenmayer.PLSystem = function () {
        ...
        this.add_command('Note(t, d, s, c, k, v, x)', function (lsystem, turtle, t, d, s, c, k, v, x) {
            ...
            turtle.note.velocity = (v * turtle.scale[5]);
            turtle.note.pan = (x * turtle.scale[6]);
            lsystem.score.append(turtle.note.clone(true));
            return turtle;
        });
        this.add_command('Note()', function (lsystem, turtle) {
            lsystem.score.append(turtle.note(true));
            return turtle;
        });
        this.add_command('Push()', function (lsystem, turtle) {
            lsystem.turtle_stack.push(turtle(true));
            return turtle;
        });
        ...
        this.add_command('T(n)', function (lsystem, turtle, n) {
            ...
            turtle.note.chord = turtle.chord.clone();
            return turtle;
        });
        this.add_command('C()', function (lsystem, turtle, n) {
            turtle.note.chord = turtle.chord.clone();
            return turtle;
        });
    ...
    ParametricLindenmayer.PLSystem.prototype.generate = function (iterations) {
        ...
        try {
            ...
            var working_turtle = this.turtle.clone(true);
    ...
    /**
     * Conforms the pitch of each event in this to the closest pitch-class of 
     * that event's chord. Assumes score is sorted by time. If a note has no 
     * chord, the last used chord is used again.
     */
    ParametricLindenmayer.PLSystem.prototype.conformToChords = function () {
        var current_chord = null;
        for (var i = 0; i < this.score.data.length; i++) {
            var event = this.score.data[i];
            if (event.chord !== null) {
                current_chord = event.chord;
            }
            if (current_chord !== null) {
                ChordSpace.conformToChord(event, current_chord, false);
            }
        }
    };
