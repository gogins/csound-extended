...
export EMCC_FLAGS='-s ALLOW_MEMORY_GROWTH=1 -s ASSERTIONS=1 -s DEMANGLE_SUPPORT=1 -s FORCE_FILESYSTEM=1 -s INITIAL_MEMORY=268435456 -s LINKABLE=1 -s NO_EXIT_RUNTIME=0 -s SAFE_HEAP=0 -s WASM=1'
...
em++ ${CXX_FLAGS} ${EMCC_FLAGS}  -iquote ../src -I../../dependencies/csound/include -I../../dependencies/csound/H -I../../dependencies/csound/interfaces -I../deps/libsndfile-1.0.25/src -c ../src/csound_embind.cpp -Iinclude -o csound_web_audio.bc

echo "Compiling CsoundAudioProcessor..."

em++ ${CXX_FLAGS} -O1 ${EMCC_FLAGS} --bind -s EXTRA_EXPORTED_RUNTIME_METHODS='["ccall", "cwrap"]' -s RESERVED_FUNCTION_POINTERS=1 -s SINGLE_FILE=1 -s WASM_ASYNC_COMPILATION=0 --source-map-base . --embed-file ../../dependencies/csound/samples@/ --pre-js ../src/CsoundAudioProcessor_prejs.js --post-js ../src/CsoundAudioProcessor_postjs.js  csound_web_audio.bc csound/libcsound.a ../deps/lib/libsndfile.a ../deps/lib/libogg.a ../deps/lib/libvorbis.a ../deps/lib/libvorbisenc.a ../deps/lib/libFLAC.a -o CsoundAudioProcessor.js
